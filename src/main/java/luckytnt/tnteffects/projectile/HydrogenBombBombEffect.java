...
public class HydrogenBombBombEffect extends PrimedTNTEffect implements NuclearBombLike {
	...
	public void serverExplosion(IExplosiveEntity ent) {
		if(ent.getLevel() instanceof ServerLevel) {
			PacketDistributor.sendToPlayersTrackingEntity((Entity)ent, new ClientboundHydrogenBombPacket(((Entity)ent).getId()));
		}
		...
		for(LivingEntity living : list) {
			living.addEffect(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(EffectRegistry.CONTAMINATED_EFFECT.get()), 4800, 0, true, true, true));
		}
	}
